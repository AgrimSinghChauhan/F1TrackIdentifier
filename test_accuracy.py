...
from f1_circuit_gui import F1CircuitIdentifier
...
class AccuracyTester:
    ...
    def test_exact_country_matches(self) -> Tuple[int, int, float]:
        ...
        for circuit_id, circuit_data in self.identifier.circuits.items():
            ...
            result = self.identifier._analyze_by_hints_only({"country": country})
            ...
            if (result.get("primary_guess") and 
                ...
                self.log_failure("Exact Country", {"country": country}, 
                               circuit_data["name"], result.get("primary_guess", {}).get("track_name"))
    ...
    def test_exact_name_matches(self) -> Tuple[int, int, float]:
        ...
        for circuit_id, circuit_data in self.identifier.circuits.items():
            ...
            result = self.identifier._analyze_by_hints_only({"name": circuit_name})
            ...
            if (result.get("primary_guess") and 
                ...
                self.log_failure("Exact Name", {"name": circuit_name}, 
                               circuit_name, result.get("primary_guess", {}).get("track_name"))
    ...
    def test_city_region_matches(self) -> Tuple[int, int, float]:
        ...
        for circuit_id, circuit_data in self.identifier.circuits.items():
            ...
            result = self.identifier._analyze_by_hints_only({
                "country": country,
                "city": city_region
            })
    ...
    def test_alias_recognition(self) -> Tuple[int, int, float]:
        ...
        for alias, expected_circuit in alias_tests:
            result = self.identifier._analyze_by_hints_only({"name": alias})
            ...
            if (result.get("primary_guess") and 
                ...
                self.log_failure("Alias", {"name": alias}, 
                               expected_circuit, result.get("primary_guess", {}).get("track_name"))
    ...
    def test_grand_prix_matches(self) -> Tuple[int, int, float]:
        ...
        for circuit_id, circuit_data in self.identifier.circuits.items():
            ...
            result = self.identifier._analyze_by_hints_only({"grand_prix": gp_name})
    ...
    def test_partial_name_matching(self) -> Tuple[int, int, float]:
        ...
        for partial_name, expected_circuit in partial_tests:
            result = self.identifier._analyze_by_hints_only({"name": partial_name})
    ...
    def test_edge_cases(self) -> Tuple[int, int, float]:
        ...
        for hints, expected_result in edge_cases:
            ...
            if expected_result is None:
                ...
                if (not result.get("primary_guess") or 
                    result["confidence_score"] < 0.3):
                    passed += 1
                else:
                    self.log_failure("Edge Case", hints, "No match", 
                                   result.get("primary_guess", {}).get("track_name"))
